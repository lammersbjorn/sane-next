...
func runDoctor(args []string) (commandResult, error) {
	root, err := lifecycleRoot("doctor", args)
	if err != nil {
...
func runRepair(args []string) (commandResult, error) {
	root, err := lifecycleRoot("repair", args)
	if err != nil {
		return commandResult{}, err
	}
	if err := requireOwnedInstall(root); err != nil {
...
func runUpdate(args []string) (commandResult, error) {
	root, err := lifecycleRoot("update", args)
	if err != nil {
		return commandResult{}, err
	}
	if err := requireOwnedInstall(root); err != nil {
	...
	if err := os.WriteFile(filepath.Join(root, "packs", "VERSION"), []byte(version+"\n"), 0o644); err != nil {
		return commandResult{}, fmt.Errorf("write pack version: %w", err)
	}
...
func runUninstall(args []string) (commandResult, error) {
	root, err := lifecycleRoot("uninstall", args)
	if err != nil {
		return commandResult{}, err
	}
	if err := requireOwnedInstall(root); err != nil {
...
func lifecycleRoot(name string, args []string) (string, error) {
